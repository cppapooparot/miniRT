    ...
    double *t_hit)
{
    ...
    disk_center = vec3_add(cylinder->center, vec3_scale(ctx->axis, ctx->half_h));
    if (intersect_disk(ray, disk_center, ctx->axis, ctx->radius, &t_disk))
        return (*t_hit = t_disk, true);
    disk_center = vec3_subtract(cylinder->center,
            vec3_scale(ctx->axis, ctx->half_h));
    ...
    double radius, double *t_hit)
{
    ...
    if (fabs(d.denom) < EPSILON)
        return (false);
    d.t = vec3_dot(vec3_subtract(disk_center, ray.origin), disk_normal) / d.denom;
    if (d.t <= EPSILON)
        return (false);
